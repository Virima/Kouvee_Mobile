...
public class Search_Filter_Detail_TransaksiProduk extends Filter {
    Adapter_Detail_TransaksiProduk adapter;
    ArrayList<TransaksiProduk_Model> filterList;

    public Search_Filter_Detail_TransaksiProduk(ArrayList<TransaksiProduk_Model>
                                                        filterList, Adapter_Detail_TransaksiProduk adapter)
    {
    ...
    protected Filter.FilterResults performFiltering(CharSequence constraint) {
        ...
        if(constraint != null && constraint.length() > 0)
        {
            ...
            {
                //CHECK
                if(filterList.get(i).getId_produk().toUpperCase().contains(constraint))
                {
