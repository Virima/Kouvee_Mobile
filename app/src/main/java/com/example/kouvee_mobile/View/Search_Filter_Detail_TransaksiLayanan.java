...
public class Search_Filter_Detail_TransaksiLayanan extends Filter {
    Adapter_Detail_TransaksiLayanan adapter;
    ArrayList<TransaksiLayanan_Model> filterList;

    public Search_Filter_Detail_TransaksiLayanan(ArrayList<TransaksiLayanan_Model>
                                                        filterList, Adapter_Detail_TransaksiLayanan adapter)
    {
    ...
    protected Filter.FilterResults performFiltering(CharSequence constraint) {
        ...
        if(constraint != null && constraint.length() > 0)
        {
            ...
            {
                //CHECK
                if(filterList.get(i).getId_layanan().toUpperCase().contains(constraint))
                {
